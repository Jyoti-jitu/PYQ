...
import {
    ...
    ChevronRight, Search, Filter, Download, X, FolderPlus, Menu
} from 'lucide-react';
...
const SearchPage = () => {
    ...
    // Listen for changes to the URL's search query from the Global Nav and update filter
    useEffect(() => {
    ...
    const fetchPyqs = async () => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL}/pyq`);
            setAllPapers(res.data.pyqs || []);
        } catch (error) {
    ...
    // Extract unique branches, years, and resources for filter dropdowns
    const uniqueBranches = ['All', ...new Set(allPapers.map(p => p.branch))];
    const uniqueYears = ['All', ...new Set(allPapers.map(p => p.year))].sort((a, b) => b - a);
    const uniqueResources = ['All', ...new Set(allPapers.map(p => p.resource_type))];

    // Apply all filters
    const filteredPapers = allPapers.filter(paper => {
        // Keyword Search
        const tagsArray = Array.isArray(paper.tags) ? paper.tags :
            (typeof paper.tags === 'string' ? JSON.parse(paper.tags || '[]') : []);
        const searchLower = searchTerm.toLowerCase();
        const matchesSearch = !searchTerm ||
            paper.title?.toLowerCase().includes(searchLower) ||
            tagsArray.some(tag => tag?.toLowerCase().includes(searchLower)) ||
            paper.branch?.toLowerCase().includes(searchLower) ||
            paper.year?.toString().includes(searchLower) ||
            paper.resource_type?.toLowerCase().includes(searchLower) ||
            paper.students?.full_name?.toLowerCase().includes(searchLower) ||
            paper.students?.roll_number?.toLowerCase().includes(searchLower);
        // Exact Match Fields
        const matchesBranch = filterBranch === 'All' || paper.branch === filterBranch;
        const matchesYear = filterYear === 'All' || paper.year === filterYear;
        const matchesResource = filterResource === 'All' || paper.resource_type === filterResource;

        return matchesSearch && matchesBranch && matchesYear && matchesResource;
    });
        ...
        <div className="min-h-screen bg-portalBgLight flex font-sans">
            ...
            <aside className={`w-64 bg-white border-r border-gray-200 flex-col shadow-sm z-50 fixed md:static inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 flex`}>
                ...
                <nav className="flex-1 p-4 flex flex-col gap-2">
                    ...
                    <button onClick={() => { navigate('/search'); setIsMobileMenuOpen(false); }} className="flex items-center space-x-3 px-4 py-3 bg-blue-50 text-portalBlue rounded-lg font-medium transition-colors w-full text-left">
                        <Search size={20} />
            ...
            {/* Main Content */}
            <main className="flex-1 flex flex-col overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 relative mb-16 md:mb-0 w-full">
                ...
                <header className="bg-white/80 backdrop-blur-sm border-b border-gray-200 py-4 px-4 sm:px-8 flex items-center justify-between sticky top-0 z-10 shadow-sm">
                    <div className="flex items-center md:hidden space-x-3 w-full">
                        ...
                        <div className="flex-1">
                            <h2 className="text-lg font-bold text-portalBlue tracking-tight truncate">Search</h2>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-4">
                        <button onClick={() => navigate('/profile')} className="text-gray-400 hover:text-portalBlue transition-colors group relative hidden sm:block">
                            <UserCircle size={28} />
                        </button>
                    </div>
                </header>
                ...
                <div className="flex-1 overflow-auto p-4 sm:p-8 relative z-10">
                    <div className="max-w-6xl mx-auto space-y-6">
                        ...
                        <div>
                            ...
                            <p className="text-gray-500 text-sm">Find exactly what you need with advanced filters.</p>
                        </div>

                        {/* Search and Filters Mastery Container */}
                        <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 flex flex-col gap-5">
                            {/* Top row: Primary Search Bar */}
                            <div className="relative">
                                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <Search size={20} className="text-portalBlue" />
                                </div>
                                <input
                                    type="text"
                                    placeholder="Search by keywords, subject, subject code, or tags..."
                                    className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-base focus:outline-none focus:ring-2 focus:ring-portalBlue/30 focus:border-portalBlue focus:bg-white text-gray-900 font-medium transition-all shadow-inner"
                                    value={searchTerm}
                            ...
                            {/* Bottom row: Dropdowns + Uploader Search */}
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Branch</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <Filter size={16} className="text-gray-400" />
                                        </div>
                                        <select
                                            className="w-full pl-9 pr-8 py-3 bg-white border border-gray-200 rounded-xl text-sm appearance-none focus:outline-none focus:border-portalBlue focus:ring-1 focus:ring-portalBlue transition-all font-medium text-gray-700 shadow-sm"
                                            value={filterBranch}
                                ...
                                <div className="space-y-1">
                                    ...
                                    <select
                                        className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm appearance-none focus:outline-none focus:border-portalBlue focus:ring-1 focus:ring-portalBlue transition-all font-medium text-gray-700 shadow-sm"
                                        value={filterYear}
                                ...
                                <div className="space-y-1">
                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Resource Type</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        ...
                                        >
                                            {uniqueResources.map(resource => (
                                                <option key={resource} value={resource}>{resource === 'All' ? 'All Types' : resource}</option>
                                            ))}
                                        </select>
                                    </div>
                                </div>
                            ...
                            {/* Active filter count badges could go here if needed in the future */}
                        </div>
                            ...
                            <div className="divide-y divide-gray-100">
                                {loadingPapers ? (
                                    <div className="p-12 text-center text-gray-500 space-y-4">
                                        ...
                                        <p className="font-medium animate-pulse">Searching knowledge base...</p>
                                    </div>
                                    ...
                                    <div className="p-16 text-center text-gray-500 flex flex-col items-center">
                                        ...
                                        <p className="font-medium text-gray-500 max-w-sm mb-6">Try adjusting your search terms, changing the branch, or spelling the uploader's name differently.</p>
                                        <button
                                            onClick={() => { setSearchTerm(''); setFilterBranch('All'); setFilterYear('All'); setFilterResource('All'); }}
                                    ...
                                    filteredPapers.map((paper) => (
                                        <div key={paper.id} className="p-5 sm:p-6 hover:bg-blue-50/30 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-5 group">
                                            <div className="flex items-start space-x-4 flex-1">
                                                <div className="p-3 bg-blue-50 text-portalBlue rounded-xl group-hover:scale-110 group-hover:bg-portalBlue group-hover:text-white transition-all shadow-sm">
                                                    <FileText size={24} />
                                                </div>
                                                <div className="flex-1">
                                                    ...
                                                    <div className="mt-2.5 flex flex-wrap items-center gap-2 text-xs font-bold text-gray-600">
                                                        ...
                                                        <div className="hidden sm:flex items-center text-gray-400 pl-2">
                                                    ...
                                                    <div className="mt-2.5 flex items-center text-xs font-medium text-gray-500 bg-gray-50 px-2.5 py-1.5 rounded-lg w-fit border border-gray-100">
                                                        ...
                                                        {paper.students?.roll_number && (
                                                            <span className="ml-1 text-gray-400">({paper.students.roll_number})</span>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center space-x-2 w-full md:w-auto mt-2 md:mt-0 pt-3 md:pt-0 border-t border-gray-100 md:border-t-0 pl-14 md:pl-0">
                                                ...
                                                <a
                                                    href={paper.file_url}
                                                    onClick={() => handleLogView(paper.id)}
                                                    download
                                                    target="_blank"
